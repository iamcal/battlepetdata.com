<?php
	...
	function db_insert($tbl, $hash, $opts=array()){			return _db_insert($tbl, $hash, 'main', null, $opts); }
	function db_insert_users($k, $tbl, $hash, $opts=array()){	return _db_insert($tbl, $hash, 'users', $k, $opts); }
	...
	function db_write($sql, $opts=array()){			return _db_write($sql, 'main', null, $opts); }
	function db_write_users($k, $sql, $opts=array()){	return _db_write($sql, 'users', $k, $opts); }

	function db_tickets_write($sql){		return _db_write($sql, 'tickets', null, array()); }
	...
	function _db_query($sql, $cluster, $k, $opts){
	...
	function _db_insert($tbl, $hash, $cluster, $shard, $opts){

		$fields = array_keys($hash);

		return _db_write("INSERT INTO $tbl (`".implode('`,`',$fields)."`) VALUES ('".implode("','",$hash)."')", $cluster, $shard, $opts);
	}

	#################################################################

	function _db_insert_dupe($tbl, $hash, $hash2, $cluster, $shard, $opts){
		...
		return _db_write("INSERT INTO $tbl (`".implode('`,`',$fields)."`) VALUES ('".implode("','",$hash)."') ON DUPLICATE KEY UPDATE ".implode(', ',$bits), $cluster, $shard, $opts);
	}

	#################################################################

	function _db_insert_bulk($tbl, $hashes, $batch_size, $cluster, $shard, $opts){
		...
		while (count($hashes)){
			...
			$ret = _db_write("INSERT{$flags} INTO $tbl (`".implode('`,`',$fields)."`) VALUES $all_values", $cluster, $shard, $opts);
	...
	function _db_update($tbl, $hash, $where, $cluster, $shard, $opts){
		...
		return _db_write("UPDATE $tbl SET ".implode(', ',$bits)." WHERE $where", $cluster, $shard, $opts);
	}
	...
	function _db_fetch($sql, $cluster, $k, $opts){

		$ret = _db_query($sql, $cluster, $k, $opts);
	...
	function _db_write($sql, $cluster, $k, $opts){
